...
import { salemen } from '../Salefiles/SaleDatamen';
import Counter from '../Counterfiles/Counter';


function ProductDetailsalemen() {
  const [cardData,setCardData]=useState({})
  const params=useParams();
useEffect(()=>{
  const data=salemen.filter((item)=>item.id==params.id) 
  setCardData(data[0])
},[])
...
export default ProductDetailsalemen;
