...
import { salewomen } from '../Salefiles/SaleDatawomen';
import Counter from '../Counterfiles/Counter';


function ProductDetailsalewomen() {
  const [womenData,setWomenData]=useState({})
  const params=useParams();
useEffect(()=>{
  const data=salewomen.filter((item)=>item.id==params.id) 
  setWomenData(data[0])
},[])
  ...
  return (
        ...
        {/* image */}
          ...
          <img src={womenData.image} alt="No Image Available" style={{borderRadius:'10px',width:'320px',height:'500px',boxShadow:'5px 5px 5px #808080'}} />
          ...
          <div className="title-description" style={{marginBottom:'50px'}}>
            ...
            {/* title */}
          <h3 style={{color:'#404040'}}><i>{womenData.title}</i></h3>
    
          {/* description */}
          <h5 style={{color:'#808080'}}><i>{womenData.description}</i></h5>
    
          {/* price */}
          <h6 className='mt-4'>{womenData.price}</h6>
...
export default ProductDetailsalewomen;
